...
const App = () => {
  // state: user
  const [userId, setUserId] = useState(localStorage.get("idUser") || ""); // To get idUser from LS or keep default value of idUser ("")
  ...
  */
  const sendLoginToApi = (loginData) => {
    ...
    apiUser.sendLoginToApi(loginData).then((response) => {
      if (response.success === true) {
        ...
        localStorage.set("idUser", response.userId); // .set("key", value)
  ...
  // 21. Bonus: Maintain user's login
  // useEffect(() => {
